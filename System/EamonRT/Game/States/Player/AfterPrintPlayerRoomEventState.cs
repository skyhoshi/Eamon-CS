	...
	public class AfterPrintPlayerRoomEventState : State, IAfterPrintPlayerRoomEventState
	{
		...
		public override void Execute()
		{
			if (!Globals.CommandPromptSeen || ShouldPreTurnProcess())
			{
				...
				gGameState.AfterPrintPlayerRoomEventHeap.PeekMin(ref eventTurn, ref eventName);

				while (!ExitEventLoop && !string.IsNullOrWhiteSpace(eventName) && eventTurn <= gGameState.CurrTurn)
				{
					gGameState.AfterPrintPlayerRoomEventHeap.RemoveMin(ref eventTurn, ref eventName);
